...
function showafter(features::Array{Float64,2},labels::Array{Float64,1},Ws::Vector{Any}, bs::Vector{Any}, netdefinition::Array{Int,1})
  # when we are training an SVM to do the classification,
  # the hypothesis of it is getting a decision boundary wiht a margin as large as possible,
  # and it do indeed results a decision boundary for classification problem, however
  # there is no sense of plotting the non-linear decision boundary of the neural networks instead taking a grid of
  # random genarated points and feed it in the neural networks, to scatter each class area,
  # and then the boundary of two classes will be clear
  # Because as for the classification, features are all labeled,
  # and our hypothesis outputs probability not like SVM outputs values such as {0,1}

  X = rand(-2.5:1e-4:3,5000,2) # taking a grid of 5000 random genarated points and feed it in the neural networks
  ...
  idx0 = find(c.==0)
  idx1 = find(c.==1)
  scatter(X[idx0,1], X[idx0,2], s=2pi, c="y", marker="o")
  scatter(X[idx1,1], X[idx1,2], s=2pi, c="m", marker="x") # scatter two classes
  legend(["Class 0", "Class 1", "Class 0", " Class 1"])   # boundary of two labeled area is the neural networks decision boundary
  axis([-2.5, 3, -2.5, 3])
  return nothing::Void
end
...
# To avoid redundant codes add following function
...
function hypothesis(X::Array{Float64,2}, theta::Vector{Float64}, netdefinition::Array{Int,1})
  theta1, theta2 = parameters(theta, netdefinition)
  m = size(X, 1)
  a1 = [ones(m, 1) X]
...
# To evaluate the gradient calculated by backpropagation
...
# Applying backpropagation algorithm to get gradient of the loss function
function nnlossgrad(theta::Array{Float64,1}, storage::Array{Float64,1}, X::Array{Float64,2}, y::Array{Float64,1}, netdefinition::Array{Int, 1})
  # first caculate theta1 and theta2 for three layer neural networks
  theta1, theta2 = parameters(theta, netdefinition)
  I = netdefinition[2]
  m = size(X, 1)
  grad = storage
  ...
  for t = 1:m
      ...
      a_3 = sigmoid(z_3)
      delta_3 = (a_3 - y[t,:]')
      delta_2 = delta_3 * theta2[:,2:end] .* dsigmoid_dz(z_2)
      Delta_1 = Delta_1 + delta_2' * a_1
      Delta_2 = Delta_2 + delta_3' * a_2
  end

  lambda = 0 # regularized parameter = 0 i.e. No regularisation
  ...
  #Unroll gradients
  L1 = length(theta1) # first L elements are gradients of theta1
  L2 = length(theta2) # rest elements are gradients of theta2
  Theta1 = Theta1_grad[:]
  Theta2 = Theta2_grad[:]
  grad[1:L2-1] = Theta1[1:L2-1]
  grad[L2+1:L1+1] = Theta1[L2:L1]
  grad[L2] = Theta2[1]
  grad[L1+2:end] = Theta2[2:end]
  storage = grad
  return storage::Array{Float64,1}
end
...
function N(netdefinition::Array{Int,1})
  n1 = netdefinition[2] + netdefinition[3]
...
function problem2()
  ...
  Ws,bs = train(trainfeatures,trainlabels, [50,40,1]) #Try i) different numbers of layers and
  ...
  # 3 perceptrons in the second layer get training error: 0.0% and testing error: 1.67%
  # 5 perceptrons in the second layer, the best iteration get training error: 0.0% and testing error: 0.83%
  # 10 perceptrons in the second layer, the best iteration get training error: 0.0% and testing error: 0.83%
  # 20 perceptrons in the second layer get result with training error: 0.0% and testing error: 0.83%
  # 40 perceptrons in the second layer get result with training error: 0.0% and testing error: 0.83%
  # 50 perceptrons in the second layer get result with training error: 0.0% and testing error: 0.83%
  # 60 perceptrons in the second layer get result with training error: 0.0% and testing error: 0.83%
  # 100 perceptrons in the second layer get the result training error: 0.0% and testing error: 1.67%
